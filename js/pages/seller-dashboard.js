...
// Function to refresh subscription data after payment
async function refreshSubscriptionAfterPayment() {
    console.log('Refreshing subscription data after payment confirmation...');
    
    // Clear pending flag
    const pendingTier = localStorage.getItem(`mbare_pending_upgrade_${currentSellerId}`);
    localStorage.removeItem(`mbare_pending_upgrade_${currentSellerId}`);
    ...
    if (sub && sub !== 'free') {
        ...
        // Re-render everything with the new tier
        renderTiers();
        updateExpiryBanner();
        updateSubscriptionControls();
        enforceProductLimit();
        
        showToast(`Successfully upgraded to ${tierMap[currentTier].name}! 🎉`);
        console.log(`Subscription upgraded to: ${currentTier}`);
    } else {
        // If still free, check if we have the pending tier stored
        if (pendingTier) {
            // Manually set the tier since the server might not have updated yet
            ...
            updateSubscriptionControls();
            enforceProductLimit();
            showToast(`Subscription updated to ${tierMap[currentTier].name}! 🎉`);
        }
    }
}
...
window.initializeDashboardData = function() {
    ...
    // When called, this means the status is 'Paid' - refresh subscription data
    refreshSubscriptionAfterPayment().then(() => {
        // Then initialize the dashboard fully if not already
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!token || !sellerId) {
        console.log("Sync skipped: missing seller credentials in localStorage.");
        document.getElementById('loadingOverlay').style.display = 'none';
        return;
    }

    // Check if there's a pending upgrade
    const pending = localStorage.getItem(`mbare_pending_upgrade_${sellerId}`);
    
    if (pending) {
        console.log(`Pending upgrade to "${pending}" detected. Waiting for payment confirmation...`);
        let pollCount = 0;
        const maxPolls = 150; // 150 * 4 seconds = 10 minutes max wait
        
        async function checkVerificationLoop() {
            if (!localStorage.getItem("seller_session_token") || !localStorage.getItem("seller_id")) {
                document.getElementById('loadingOverlay').style.display = 'none';
                return;
            }
            ...
            if (pollCount > maxPolls) {
                console.log("Max polling attempts reached. Initializing with current status.");
            ...
            try {
                console.log(`Polling database for subscription verification update (attempt ${pollCount})...`);
                const response = await fetch(`/api/subscriptions/status/${sellerId}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
                if (response.ok) {
                    const data = await response.json();
                    console.log("Current verification status:", data.status);
                    
                    if (data.status === 'Paid') {
                        console.log("Status confirmed as 'Paid'. Upgrading subscription...");
                        // Clear pending flag and update subscription
                        const pendingTier = localStorage.getItem(`mbare_pending_upgrade_${sellerId}`);
                        localStorage.removeItem(`mbare_pending_upgrade_${sellerId}`);
                        
                        // Set the tier to the pending tier
                        if (pendingTier && pendingTier !== 'free') {
                            currentTier = pendingTier;
                            localStorage.setItem(`mbare_tier_${sellerId}`, currentTier);
                            
                            // Fetch fresh subscription data
                            await fetchSubscription();
                            
                            // Initialize dashboard with new tier
                            await init();
                            
                            showToast(`Successfully upgraded to ${tierMap[currentTier].name}! 🎉`);
                        } else {
                            // If no pending tier, just initialize
                            await init();
                        }
                        return;
                    }
                ...
                console.error("Observer synchronization tracking error:", e);
            }
            // Continue polling
            setTimeout(checkVerificationLoop, 4000);
        }

        // Start polling for payment confirmation
        checkVerificationLoop();
    } else {
        // No pending upgrade - initialize immediately
        console.log("No pending upgrade. Initializing dashboard immediately.");
        init();
    }
});
